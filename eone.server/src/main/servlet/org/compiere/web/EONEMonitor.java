...
import eone.server.EONEServerGroup;
import eone.server.EONEServerMgr;
import eone.server.IServerManager;
import eone.server.LogFileInfo;
import eone.server.ServerCount;
import eone.server.ServerInstance;
import eone.server.SystemInfo;
import eone.server.TrxInfo;
import eone.server.cluster.ClusterServerMgr;
import eone.server.cluster.callable.DeleteLogsCallable;
import eone.server.cluster.callable.GetLogInfoCallable;
import eone.server.cluster.callable.ReadLogCallable;
import eone.server.cluster.callable.RotateLogCallable;
import eone.server.cluster.callable.SetTraceLevelCallable;
 ...
 */
public class EONEMonitor extends HttpServlet
{
	/**
	 * 
	 */
	...
	/**	Logger				*/
	private static final CLogger	log = CLogger.getCLogger(EONEMonitor.class);
	/**	The Server			*/
		...
		throws ServletException, IOException
	{
		...
		WebDoc doc = WebDoc.create ("EONE Server Monitor Log");
		//	Body
		body b = doc.getBody();
		//
		p para = new p();
		a link = new a ("eoneMonitor#" + serverID, "Return");
	...
	//	log.info("ServletConfig=" + getServletConfig());
			...
			if (members.size() > 1 && local != null) 
			{
				...
				for(IClusterMember member : members) 
				{
					...
					else 
					{
						...
						a link = new a ("eoneMonitor?NodeInfo="+member.getId(), nodeBuilder.toString());
						para.addElement(link);
					}					
				}
		...
		//
		p para = new p();
		a link = new a ("eoneMonitor?Action=Start_All", "Start All");
		para.addElement(link);
		para.addElement(" - ");
		link = new a ("eoneMonitor?Action=Stop_All", "Stop All");
		para.addElement(link);
		para.addElement(" - ");
		link = new a ("eoneMonitor?Action=Reload", "Reload");
		para.addElement(link);
		para.addElement(" - ");
		link = new a ("eoneMonitor", "Refresh");
		para.addElement(link);
		bb.addElement(para);
		...
		for (int i = 0; i < servers.length; i++)
		{
			...
			{
				String msg = "Stop";
				link = new a ("eoneMonitor?Action=Stop_" + server.getServerId(), msg);
			...
			else
			{
				...
				link = new a ("eoneMonitor?Action=Start_" + server.getServerId(), msg);
				line.addElement(new td().addElement(link));
			}
			table.addElement(line);
			//
			...
			td.addElement(" - ");
			link = new a ("eoneMonitor?RunNow=" + server.getServerId(), "(Run Now)");
			td.addElement(link);
			line.addElement(td);
			table.addElement(line);
			//
			...
			bb.addElement(link);
			bb.addElement(" - ");
			link = new a ("eoneMonitor?Log=" + server.getServerId(), "Log");
			bb.addElement(link);
			bb.addElement(" - ");
			link = new a ("eoneMonitor", "Refresh");
			bb.addElement(link);
	...
	private void createLogMgtPage (body bb, Collection<IClusterMember> members, IClusterMember local)
	{
		...
		p cachePara = new p();
		cachePara.addElement(new a ("eoneMonitor?CacheReset=Yes", "Reset Cache"))
			.addElement(" - ")				
			.addElement(new a ("eoneMonitor?CacheDetails=Yes", "Cache Details"));
		...
		form myForm = new form("eoneMonitor", form.METHOD_POST, form.ENC_DEFAULT);
		//	LogLevel Selection
		...
		line.addElement(new td().addElement(new a ("eoneMonitor?Trace=" + systemInfo.getCurrentLogFile(), "Current")));
		table.addElement(line);
		//
		line = new tr();
		p tlp = new p();
		tlp.addElement(new a ("eoneMonitor?Trace=ROTATE", "Rotate Trace Log"))
		   .addElement(" - ")
		   .addElement(new a ("eoneMonitor?Trace=DELETE", "Delete all Trace Logs"));
		...
		for (LogFileInfo logFile : logFiles) 
		{
			...
			a link = new a ("eoneMonitor?Trace=" + fileName, fileName);
			p.addElement(link);
		...
		for (int i = 0; i < clients.length; i++)
		{
			...
			if (i > 0)
				p.addElement(" - ");
			p.addElement(new a("eoneMonitor?EMail=" + client.getAD_Client_ID(), client.getName()));
		}
		if (clients.length == 0)
			p.addElement("&nbsp;");
		...
		throws ServletException, IOException
	{
		...
		//
		p para = new p();
		a link = new a ("eoneMonitor", "Return");
		para.addElement(link);
		b.addElement(para);
		//
		table table = new table();
	...
	private void createNodeInfoPage (body bb, String nodeId)
	{
		...
		line = new tr();
		line.addElement(new th().addElement("Trace File"));
		line.addElement(new td().addElement(new a ("eoneMonitor?Trace=" + systemInfo.getCurrentLogFile()
			 + "&nodeId=" + nodeId, "Current")));
		table.addElement(line);
		//
		line = new tr();
		p tlp = new p();
		tlp.addElement(new a ("eoneMonitor?Trace=ROTATE&nodeId="+nodeId, "Rotate Trace Log"))
		  .addElement(" - ")
		  .addElement(new a ("eoneMonitor?Trace=DELETE&nodeId="+nodeId, "Delete all Trace Logs"));
		line.addElement(new th());
		line.addElement(new td().addElement(tlp));
		table.addElement(line);
		//
		...
		for (LogFileInfo logFile : logFiles) 
		{
			if (logFile != logFiles[0])
				p.addElement(" - ");
			String fileName = logFile.getFileName();
			a link = new a ("eoneMonitor?Trace=" + fileName + "&nodeId="+nodeId, fileName);
			p.addElement(link);
			int size = (int)(logFile.getFileSize()/1024);
			if (size < 1024)
				p.addElement(" (" + size + "k)");
