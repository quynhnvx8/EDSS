...
public class EONEServerMgr implements ServiceTrackerCustomizer<IServerFactory<EONEServer, EONEProcessor>, IServerFactory<EONEServer, EONEProcessor>>, BundleListener, IServerManager
{
	...
	public synchronized static EONEServerMgr get(boolean createNew)
	{
		if (m_serverMgr == null && createNew)
		{
			...
			serviceTracker = new ServiceTracker<IServerFactory<EONEServer, EONEProcessor>, IServerFactory<EONEServer, EONEProcessor>>(EONEServerActivator.getBundleContext(), 
					IServerFactory.class.getName(), m_serverMgr);
			serviceTracker.open();
			EONEServerActivator.getBundleContext().addBundleListener(m_serverMgr);
		}
		return m_serverMgr;
	}	//	get
	...
	public synchronized String stop (String serverID)
	{
		...
		{
			server.scheduleFuture.cancel(true);
			Thread.sleep(1000);//10	//	1/100 sec
		}
		catch (Exception e)
		{
	...
	public synchronized IServerFactory<EONEServer, EONEProcessor> addingService(
		...
		IServerFactory<EONEServer, EONEProcessor> factory = EONEServerActivator.getBundleContext().getService(reference);
		createServers(factory);
		startAll();
		return factory;
	}
