...
public abstract class TabbedDesktop extends AbstractDesktop {
	...
	public ADForm openForm(int formId) {
		...
		MForm mform = new MForm(Env.getCtx(), formId, null);
		if (Window.Mode.EMBEDDED == form.getWindowMode()) {
			DesktopTabpanel tabPanel = new DesktopTabpanel();
			form.setParent(tabPanel);
			if (mform.getZoomLogic() != null && !mform.getZoomLogic().isEmpty())
				form.setZoomLogic(mform.getZoomLogic());
			if (mform.getZO_Window_ID() > 0)
				form.setZoomWindow(mform.getZO_Window_ID());
