        ...
        EventListener<Event>, DataStatusListener, ActionListener, ITabOnSelectHandler
{
	...
	public boolean initPanel(MQuery query)
    {
        ...
        //toolbar.updateToolbarAccess(adWindowId); //Bỏ cái này do reload lại cấu hình button
    ...
    public void onAppendSign()
    {
		...
		EventListener<Event> listener = new EventListener<Event>() {
			...
			@Override
			public void onEvent(Event event) throws Exception {
				toolbar.setPressed("AppendSign",adTabbox.getSelectedGridTab().hasAppendSign());
				focusToActivePanel();				
			}
		};

		WAppendSign win = new WAppendSign (curTab, curWindowNo, adTabbox.getSelectedGridTab().getAD_AppendSign_ID(),
							adTabbox.getSelectedGridTab().getAD_Table_ID(), record_ID, null, listener);		
		win.addEventListener(DialogEvents.ON_WINDOW_CLOSE, new EventListener<Event>() {
			@Override
			public void onEvent(Event event) throws Exception {
    ...
    public void onEvent(Event event)
    {
    	...
    	else if (ADTabpanel.ON_DYNAMIC_DISPLAY_EVENT.equals(event.getName()))
    	{
    		...
    		if (adtab == adTabbox.getSelectedTabpanel()) {
    			...
    			//FIXME Bỏ đoạn load button trong database
    ...
    public void dataStatusChanged(DataStatusEvent e)
    {
        ...
        boolean insertRecord = adTabbox.getSelectedGridTab().isInsertRecord(); //!readOnly
        boolean deleteRecord = adTabbox.getSelectedGridTab().isDeleteRecord(); //!readOnly;
        if (!detailTab)
        {
	        //  update Change
	        boolChanges = changed;
...
*/
	...
	private void executeButtonProcess0(final IProcessButton wButton,
		...
		else if (pr.getZO_Window_ID() > 0 && pr.getClassname().isEmpty()) {
			AEnv.zoom(table_ID, record_ID, pr.getZO_Window_ID());
		}
		else
		{
			IADTabpanel adtabPanel = null;
	...
	public void onComplete()
    {
        ...
        List<X_AD_Signer> data = MAppendSign.getListSign(Env.getCtx(), curTab.getAD_Table_ID(), record_ID, null);
        if (data != null && data.size() > 0) {
        	final WSignPanel win = new WSignPanel(curTab, data);
        	getComponent().getParent().appendChild(win);
			showBusyMask(win);
			LayoutUtils.openOverlappedWindow(getComponent(), win, "middle_center");
			win.addEventListener(DialogEvents.ON_WINDOW_CLOSE, new EventListener<Event>() {
	...
	public void validateAPCA() {
		...
		toolbar.setVisible("AppendSign", false);
		
        if (fieldApproved != null && fieldApproved.isDisplayed()) {
        	boolean hasDocStatus = fieldStatus != null;
        	boolean isComplete = hasDocStatus && !DocAction.STATUS_Drafted.equals(fieldStatus.getValue());
        ...
        if (fieldApproved != null && fieldApproved.isDisplayed()) {
        	boolean isComplete = DocAction.STATUS_Completed.equals(fieldStatus.getValue());
    		boolean isShowAcct = Env.getContext(Env.getCtx(), "#ShowAcct") == "Y" ? true : false;
    		boolean hasPosted = isComplete && isShowAcct;
        ...
        //Button Sign
        if (fieldStatus!= null &&  curTab.isSignFlow()) {
        	toolbar.setVisible("AppendSign", true);
        }
	}
}
