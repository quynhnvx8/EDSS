...
DataStatusListener, IADTabpanel, IdSpace, IFieldEditorContainer
{
	...
	private WEditor	currentFocusField = null;
    ...
    protected void createUI(boolean update)
    {
        ...
        for (int i = 0; i < fields.length; i++)
        {
        	...
        	if (field.isToolbarButton()) {
        		...
        		if (editor != null) {
        			//Cac button nay ko add vao process tren toolbar => Process chi dung cho report
        			if ("Posted".equalsIgnoreCase(field.getColumnName())
        					|| "Approved".equalsIgnoreCase(field.getColumnName())
        					|| "Canceled".equalsIgnoreCase(field.getColumnName())) {
        				continue;
        			}
...
//        			rowg.appendChild(cell);
        	...
        	if (field.isHeading())
        		actualxpos = xpos;
        	else if (rowSpan > 1 && xpos + columnSpan == xposImage) {
        		actualxpos = xpos + columnSpan - 1 + colSpan;
        		rowSpan --;
        	}
        	else
        	...
        	//Xu ly ActualPost o day doi voi Field la kieu Image.
        	if (field.getNumLines() > 2 && field.isImage()) {
				rowSpan = field.getNumLines() - 1;//Danh dau so lan can add vao actualPost
				xposImage = xpos; //Danh dau bat dau Rowspan
				colSpan = columnSpan; //Danh dau so cot can add vao actualPost
			}
        	//System.out.println("Column: " + field.getColumnName() + ", columnSpan: " + columnSpan + ",xpos:" + xpos + ", actualPos: " + actualxpos);
        	if (! field.isHeading()) {
        		...
        		if (editor != null) // Not heading
        		{
    				...
    				divs.appendChild(div);
    				if (field.getNumLines() > 2 && field.isImage())
    					row.appendCellChild(divs, columnSpan, rowSpan);
    				else
    					row.appendCellChild(divs, columnSpan);
        			...
        			if (defaultFocusField == null && field.isDefaultFocus()) {
        				...
        				currentFocusField = defaultFocusField;
        			}
	...
	private void addNewNode() {
    	if (gridTab.getRecord_ID() > 0) {
			...
			MTreeNode node = new MTreeNode (gridTab.getRecord_ID(), name, description, parentID, summary, imageIndicator, null);
	...
	public boolean onEnterKey() {
		if (listPanel.isVisible()) {
			...
			focusToNextEditor(currentFocusField);
			return true;
		}
		//return false;
	}
