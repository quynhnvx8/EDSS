...
import eone.webui.component.ADTabListModel;
import eone.webui.component.ADTabListModel.ADTabLabel;
...
public class CompositeADTabbox extends AbstractADTabbox
{
    	...
    	detailPane.setEventListener(new EventListener<Event>() {
			...
			private void onDeleteSelected(final IADTabpanel tabPanel) {
				...
				if(indices.length > 0) {
					...
					FDialog.ask(sb.toString(), tabPanel.getGridTab().getWindowNo(), null,Msg.getMsg(Env.getCtx(), "DeleteSelection"), new Callback<Boolean>() {
						@Override
						public void onCallback(Boolean result) {
							if(result){
	...
	private void onActivateDetail(IADTabpanel tabPanel) {
		...
		if (!tabPanel.isVisible()) {
			...
			//TODO Khong hieu sao loi o day 03/12/2020
			/*
