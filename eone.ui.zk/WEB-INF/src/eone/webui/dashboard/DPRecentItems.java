...
public class DPRecentItems extends DashboardPanel implements EventListener<Event>, EventHandler {
	...
	public DPRecentItems()
	{
		...
		{
			Image imgr = new Image(ITheme.ICON_REFRESH);
			recentItemsToolbar.appendChild(imgr);
			imgr.setSclass("img-btn");
		...
		{
			Image img = new Image(ITheme.ICON_DELETE);
			recentItemsToolbar.appendChild(img);
			img.setSclass("img-btn");
	...
	private static synchronized void createTopicSubscriber() {
		if (topicSubscriber == null) {
			...
			IMessageService service = Core.getMessageService();//Service.locator().locate(IMessageService.class).getService();
			if (service != null) {
