...
public class InvoiceHistory extends Window implements EventListener<Event>
{
	...
	public InvoiceHistory (Window parent, 
		int C_BPartner_ID, int M_Product_ID, int M_Warehouse_ID)
	{
		super();
	...
	private void initAtpTab ()
	{
		...
		//	Fill Storage Data
		String sql = "SELECT s.Qty, w.Name, p.Name "
			+ "FROM M_Storage s"
		...
		sql = "SELECT o.DatePromised, ol.QtyReserved,"
			+ " '' Attributet, ol.M_AttributeSetInstance_ID,"
		...
		try
		{
			...
			while (rs.next())
			{
				...
				if (MDocType.DOCBASETYPE_Debit_331.equals(DocBaseType))
				{
