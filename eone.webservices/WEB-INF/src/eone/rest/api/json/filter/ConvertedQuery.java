...
public class ConvertedQuery {
	...
	public void addParameter(MColumn column, String parameter) {
		...
		if (column.isSecure() || column.isEncrypted())
			throw new EONEException("Cannot query the column: " + column);
		...
		try {
			...
			else if (DisplayType.isDate(displayType)) //	Timestamps
			{
				...
				if (parameter.startsWith("'") && parameter.endsWith("'"))
					...
					throw new EONERestException("String values must be put between single quotes. ColumnName: " + column.getName(), Status.BAD_REQUEST);
			}
			else 
				addParameter(parameter);
		} catch (Exception e) {
			throw new EONERestException("Error convertig parameter with value: " + parameter + " - " + e.getMessage(), Status.BAD_REQUEST);
		}
	}
