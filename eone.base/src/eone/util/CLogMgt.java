...
public class CLogMgt
{
	...
	public static StringBuffer getInfo (StringBuffer sb)
	{
		...
		//
		sb.append("Home = ").append(Adempiere.getAdempiereHome()).append(NL);
		sb.append("Properties = ").append(Ini.getPropertyFileName()).append(NL);
		...
		*/
		...
		if (Env.getAD_Client_ID(Env.getCtx()) == 0) {
			IClusterService service = Core.getClusterService();//Service.locator().locate(IClusterService.class).getService();
			if (service != null) {
