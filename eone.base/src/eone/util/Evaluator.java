...
public class Evaluator
{
	...
	public static boolean evaluateLogic (Evaluatee source, String logic)
	{
		...
		{
			s_log.severe ("Logic does not comply with format ' => " + logic);
			return false;
		}
	...
	private static boolean evaluateLogicTuple (Evaluatee source, String logic)
	{
		...
		String[] list = rightToken.split(",");//split("[,]");
		for(String second : list)
		{
			...
			//quynhnv.x8: add multi value, ex: 
			//Compare: IF 123 vs {12,123,13...} : true ELSE {12,123,13...} vs 123 : false
			String [] secondEvals = secondEval.split(",");
			for (String secondEvalue : secondEvals) {
				secondEvalue = secondEvalue.replaceAll(" ", "");
				boolean resulteque = false;
				boolean result = evaluateLogicTuple (firstEval, operand, secondEvalue);
				if (s_log.isLoggable(Level.FINEST)) s_log.finest(logic 
						+ " => \"" + firstEval + "\" " + operand + " \"" + secondEvalue + "\" => " + result);
				//Neu toan tu khac trong 1 tap, thi can xet them co phan tu nao bang ko
				if ("!".equals(operand) && secondEval.indexOf(",") > 0) { 
					resulteque = evaluateLogicEqua(firstEval, secondEvalue);
					if (resulteque)
						return false;
					else
						continue;
				}
				if (result && ! resulteque)
					return true;				
			}			
	...
	private static boolean evaluateLogicEqua (String value1, String value2)
	{
