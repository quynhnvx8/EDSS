...
import eone.util.SecureInterface;
...
//		return DB.getSQLValueString(null, "select lastmigrationscriptapplied from ad_system");
	...
	private static void setPackageInfo()
	{
		...
		Package adempierePackage = Package.getPackage("eone");
	...
	public static boolean startupEnvironment (boolean isClient)
	{
		...
		try
		{
			...
			{
				className = System.getProperty(SecureInterface.EONE_SECURE);
				if (className != null && className.length() > 0
					&& !className.equals(SecureInterface.EONE_SECURE_DEFAULT))
				{
