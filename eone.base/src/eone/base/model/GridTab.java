...
public class GridTab implements DataStatusListener, Evaluatee, Serializable
{
	...
	private String validateQuery (MQuery query)
	{
		...
		//	Find Reference Column e.g. BillTo_ID -> 
	...
	*/
	...
	public boolean isDeleteRecord()
	{
		...
		if (!m_vo.IsDeleteable)
			return false;
		if (m_vo.DeleteLogic == null || "".equals(m_vo.DeleteLogic))
			return true;
		return Evaluator.evaluateLogic(this, m_vo.DeleteLogic);
	}	//	isDeleteRecord
	...
	public boolean canHaveAppendSign()
	{
		if (getKeyColumnName().endsWith("_ID"))
			return true;
		return false;
	...
	{
		if (!canHaveAppendSign())
			return 0;
		int recordID = m_mTable.getKeyID(m_currentRow);
		return MAppendSign.getID(m_vo.AD_Table_ID, recordID);
	}
	
	public boolean hasAppendSign()
	{
		return getAD_AppendSign_ID() > 0;
	}
	...
	public int getNumColumns() {
        ...
        for (GridField gridField : getFields())
        {
        	if (!gridField.isDisplayed() || gridField.isToolbarOnlyButton())
        		continue;
        	int col = gridField.getXPosition() + gridField.getColumnSpan() - 1;
        	/*
	...
	public int[] getRecordSelection() 
	{
		...
		for(Integer row : selection) 
		{
			selected[i++] = this.getKeyID(row.intValue());
		}
		return selected;
	}
	...
	private static CCache<Integer, List<MTabProcess>> m_Processes 
	= new CCache<Integer, List<MTabProcess>>("m_Processes", 10, CCache.DEFAULT_TIMEOUT);
	public List<MTabProcess> loadProcesses(boolean isMulti)
	{
		@SuppressWarnings("deprecation")
		Integer key = new Integer(getAD_Tab_ID());
		List<MTabProcess> processes;
		
		processes = m_Processes.get(key);
		if (processes != null && processes.size() > 0)
			return processes;
		
		processes = new Query(m_vo.ctx, MTabProcess.Table_Name, " Ad_Tab_ID = ? And IsActive = 'Y'", null)
					.setParameters(m_vo.AD_Tab_ID, "N")
					.setOnlyActiveRecords(true)
					.setOrderBy(MTabProcess.COLUMNNAME_Name)
					.list();
		
		m_Processes.put(key, processes);
		
		return processes;
		
	}
	
	public PO dataSavePO(boolean manualCmd) 
	{
		PO po = null;
		if (dataSave(manualCmd)) {
			po = m_mTable.getBufferPO();
		}
		return po;
	}

	public GridTab[] getChildrenTab() {
		ArrayList<GridTab> tabs = new ArrayList<GridTab>();

		int tabNo = m_vo.TabNo;
		int currentLevel = m_vo.TabLevel + 1;
		int childLevel = currentLevel;
		while (childLevel == currentLevel) {
			tabNo++;
			currentLevel = Env.getContextAsInt(m_vo.ctx, m_vo.WindowNo, tabNo,
					GridTab.CTX_TabLevel);
			if (childLevel == currentLevel) {
				tabs.add(m_window.getTab(tabNo));
			}
		}

		return tabs.toArray(new GridTab[tabs.size()]);
	}
	
	public List<GridTab> getAllTabs()
	{
		List<GridTab> list = new ArrayList<GridTab>(1);
		for (int i = 0; i < m_window.getTabCount(); i++)
		{
