...
//	  Do we have a mandatory rule
	...
	public boolean isEditable (Properties ctx, boolean checkContext,boolean isGrid)
	{
		...
		if ((m_vo.ColumnName.equals("Record_ID") && m_vo.displayType == DisplayType.Button))	//  Zoom
			return true;
	...
	protected Object defaultForClientOrg (){
		...
		else if (m_vo.ColumnName.equals("AD_Org_ID"))
		{
			...
			//FIXME Quynhnv.x8: 27.12.2021
			return Env.getAD_Org_ID(Env.getCtx()); //Integer.valueOf(0); 
			
		}
		
		else if (m_vo.ColumnName.equals("AD_Client_ID"))
		{
			if (log.isLoggable(Level.FINE)) log.fine("[ClientAccess] " + m_vo.ColumnName + "=0");
			//FIXME Quynhnv.x8: 27.12.2021
			return Env.getAD_Client_ID(Env.getCtx()); //Integer.valueOf(0); 
	...
	protected Object defaultFromPreference(Character defaultValueType) {
		String defStr = "";
		if (defaultValueType.equals('4')  && !m_vo.DefaultValue.isEmpty()){
			/**
			 * (d) Preference (user) - P|
			 */
		...
		}else if (defaultValueType.equals('5') && !m_vo.DefaultValue.isEmpty()){
			/**
			 * (e) Preference (System) - # $
			 */
	...
	public String getColumnSQL(boolean withAS)
	{
		...
		if (m_vo.displayType == DisplayType.Button)
			return " NULL AS " + m_vo.ColumnName; 
		return m_vo.ColumnName;
	}	//	getColumnSQL
	 ...
	 */
	private static CCache<Integer, ArrayList<String>>  s_cacheAutoLoad = new CCache<Integer, ArrayList<String>>("GridField_AutoLoad", 30, 60);	//	1h
	...
	public void updateContext() {
		...
		else
		{
			...
			if(s_cacheAutoLoad.containsKey(m_vo.AD_Field_ID)) {
				ArrayList<String> lst = s_cacheAutoLoad.get(m_vo.AD_Field_ID);
				for(String str : lst) {
					if (!isParentTabField())
					{
						Env.setContext(m_vo.ctx, m_vo.WindowNo, str, "");
					}
					Env.setContext(m_vo.ctx, m_vo.WindowNo, m_vo.TabNo, str, null);
				}
			}
			//End add
			if (!isParentTabField() && isUpdateWindowContext())
			...
			// Quynhnv.x8 added on 13/06/2020: Bo sung cac gia tri thuoc bang reference
			if (this.isLookup() && m_lookup != null)
			{
				NamePair pp = m_lookup.get(m_value);
				if (pp != null)
				{
					HashMap<String, Object> hiddenDatas = pp.getDatas();
					if (hiddenDatas != null && hiddenDatas.size() > 0)
					{
						for (String key : hiddenDatas.keySet())
						{
							...
							if(!lst.contains(m_vo.ColumnName + ":" + key))
								lst.add(m_vo.ColumnName + ":" + key);
							if(!s_cacheAutoLoad.containsKey(m_vo.AD_Field_ID)) {
								s_cacheAutoLoad.put(m_vo.AD_Field_ID, lst);
							}
							
							Object value = hiddenDatas.get(key);
							if (!isParentTabField())
							{
								Env.setContext(m_vo.ctx, m_vo.WindowNo, m_vo.ColumnName + ":" + key, 
										value==null ? null : value.toString());
							}
							
							Env.setContext(m_vo.ctx, m_vo.WindowNo, m_vo.TabNo, m_vo.ColumnName + ":" + key, 
									value==null ? null : value.toString());
						}
					}
	...
	*/
	...
	//Quynhnv.x8 add ImageCheck
	public boolean isImage()
	{
		return m_vo.displayType == DisplayType.Image;
	}
