...
public class MBPGroup extends X_C_BP_Group
{
	...
	/**	Cache						*/
	private static CCache<Integer,MBPGroup>	s_cache	= new CCache<Integer,MBPGroup>(Table_Name, 10);
	
	private static CCache<String, Integer>	s_cacheType	= new CCache<String, Integer>(Table_Name, 10);
	/**	Default Cache					*/
	private static CCache<Integer,MBPGroup>	s_cacheDefault	= new CCache<Integer,MBPGroup>(Table_Name, MBPGroup.class.getName()+".Default", 5);
	/**	Logger	*/
	private static CLogger s_log = CLogger.getCLogger (MBPGroup.class);
	
	public static int getC_BP_Group_ID(String groupType) {
		if (!s_cacheType.containsKey(groupType))
		{
			String sql = "Select C_BP_Group_ID From C_BP_Group Where GroupType = ?";
			int C_BP_Group_ID = DB.getSQLValue(null, sql, groupType);
			s_cacheType.put(groupType, C_BP_Group_ID);
		}
		return s_cacheType.get(groupType);
	}
