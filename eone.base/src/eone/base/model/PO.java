...
public abstract class PO	implements Serializable, Comparator<Object>, Evaluatee, Cloneable
{
	...
	public final boolean set_ValueOfColumnReturningBoolean (int AD_Column_ID, Object value)
	{
		int index = p_info.getColumnIndex(AD_Column_ID);
		if (index < 0)
			throw new EONEUserError("Not found - AD_Column_ID=" + AD_Column_ID);
		String ColumnName = p_info.getColumnName(index);
	 ...
	 *  @param value value
	 */
	public void set_Keys (String ColumnName, Object value)
	{
		//	Update if KeyColumn
	...
	public boolean load (String trxName)
	{
		...
		for (int i = 0; i < size; i++)
		{
			...
			if (p_info.getColumnDisplayType(i) == DisplayType.Button)
				columnSQL = " NULL AS "  + p_info.getColumnSQL(i);
	...
	{
		Integer ii = (Integer)get_Value("AD_Department_ID");
		if (ii == null)
			return 0;
		return ii.intValue();
	}
	...
	/**
	 * Quynhnv.x8: Bo sung ham check trung gia tri cac bang. Quan diem co gang code 1 cho de dung dc nhieu noi
	 * Add 23/10/2020
	...
	public static boolean isCheckDoubleValue(String tableName, Map<String, Object> dataColumn, String PrimaryKey, int Record_ID, String trxName) {
		...
		String whereClause = "";
		for(Map.Entry<String, Object> key : dataColumn.entrySet()) {
			if (key.getValue() instanceof Timestamp) {
				whereClause = whereClause + " And Trunc(" + key.getKey() + ") =  ?";				
			} else {
				whereClause = whereClause + " And " + key.getKey() + " = ?";
			}
	...
	private boolean saveFinish (boolean newRecord, boolean success)
	{
		...
		set_ValueNoCheck("UpdatedBy", Env.getAD_User_ID(getCtx()));
	...
	private boolean doUpdate(boolean withValues) {
		...
		for (int i = 0; i < size; i++)
		{
				...
				)
			{
				...
				MChangeLog cLog = new MChangeLog(getCtx(), 
						AD_ChangeLog_ID, m_trxName, sessionID,
								p_info.getAD_Table_ID(), p_info.getColumn(i).AD_Column_ID, get_ID(), getAD_Client_ID(), getAD_Org_ID(),
						null, value, MChangeLog.EVENTCHANGELOG_Update);
				if (cLog.save())
				{
	...
	private boolean saveNew()
	{
		...
		if (m_IDs.length == 1 && p_info.hasKeyColumn()
			&& m_KeyColumns[0].endsWith("_ID"))	//	AD_Language,
		{
			int no = saveNew_getID();
			if (no <= 0)
			{
				Object key = get_Value(get_TableName() + "_ID");
		...
		if (index != -1)
		{
			...
			{
				int dt = p_info.getColumnIndex("C_DocType_ID");
	...
	private boolean doInsert(boolean withValues) {
		...
		for (int i = 0; i < size; i++)
		{
					...
					|| (accessLevel == ACCESSLEVEL_All && Env.isUserSystem(getCtx()))) 
			{
				...
				if ("AD_Client_ID".equals(columnName) && Integer.parseInt(value.toString()) == 0 ) {
					value = Env.getAD_Client_ID(getCtx());
				}
				
				if ("AD_Org_ID".equals(columnName) && Integer.parseInt(value.toString()) == 0 ) {
					value = Env.getAD_Org_ID(getCtx());
				}
				...
				)
			{
				...
				MChangeLog cLog = new MChangeLog(getCtx(), 
						AD_ChangeLog_ID, m_trxName, sessionID,
								p_info.getAD_Table_ID(), p_info.getColumn(i).AD_Column_ID, get_ID(), getAD_Client_ID(), getAD_Org_ID(),
						null, value, MChangeLog.EVENTCHANGELOG_Insert);
				if (cLog.save())
				{
					AD_ChangeLog_ID = cLog.getAD_ChangeLog_ID();
				}
...
//		int dt = p_info.getColumnDisplayType(index);
		...
		try
		{
			...
			else
			{
				if (success)
				{
					...
					if( p_info.isChangeLog() && insertLog.equalsIgnoreCase("Y"))
					{
						...
						if (m_IDs.length == 1)
						{
							...
							for (int i = 0; i < size; i++)
							{
									...
									)
								{
									...
									String sessionID = Env.getContext(Env.getCtx(), "#AD_Session_ID");
									MChangeLog cLog = new MChangeLog(getCtx(), 
											AD_ChangeLog_ID, m_trxName != null ? m_trxName : localTrxName, sessionID,
											AD_Table_ID, p_info.getColumn(i).AD_Column_ID, Record_ID, getAD_Client_ID(), getAD_Org_ID(),
											value, null, MChangeLog.EVENTCHANGELOG_Delete);
									if (cLog.save())
									{
										AD_ChangeLog_ID = cLog.getAD_ChangeLog_ID();
									}
	...
	public void update_Tree (String treeType)
	{
		...
		/*else {
			sourceTableName = MTree.getSourceTableName(treeType);
			if (MTree.TREETYPE_ElementValue.equals(treeType) && this instanceof I_C_ElementValue) {
		...
		*/
		...
		List<MTree> trees = new Query(getCtx(), MTree.Table_Name, whereTree, get_TrxName())
			.setClient_ID()
	...
	protected boolean delete_Tree (String treeType)
	{
		...
		StringBuilder sb = new StringBuilder ("DELETE FROM ")
			.append(MTree.getNodeTableName(treeType))
...
//				int index = p_info.getColumnIndex(columnName);
	...
	{
		if (p_info.getColumnIndex("DocStatus") != -1)
			this.set_Value("DocStatus", "DR");
		if (p_info.getColumnIndex("Posted") != -1)
			this.set_Value("Posted", false);
		if (p_info.getColumnIndex("DocAction") != -1)
			this.set_Value("DocAction", "CO");
		if (p_info.getColumnIndex("Processed") != -1)
			this.set_Value("Processed", false);
		if (p_info.getColumnIndex("Processing") != -1)
			this.set_Value("Processing", false);
		
		String sql = "";
		int no = -1;
		
		sql = "DELETE FROM Fact_Acct WHERE AD_Table_ID = " + this.get_Table_ID()
	...
	private static CCache<Integer, String> s_SqlPoInsertOnly = new CCache<Integer, String>("SqlPoInsertOnly", 5,20);
   
	@SuppressWarnings("rawtypes")
	private static CCache<Integer, List> s_SqlPoInsert_ParaOnly = new CCache<Integer, List>("SqlPoInsert_ParaOnly", 5, 20);
	private static CCache<Integer, String> s_SqlPoUpdate = new CCache<Integer, String>("PO_SqlPoUpdate", 5, 20); // 20 mininus
	@SuppressWarnings("rawtypes")
	private static CCache<Integer, List> s_SqlPoUpdate_Para = new CCache<Integer, List>("PO_SqlPoUpdate_Para", 5,20);
	
	public String get_ValueAsString (int index)
	{
		Object value = get_Value (index);
		if (value == null)
			return "";
	...
	public static List<Object> getValues (PO po, List<String> colNames) 
	{
		List<Object> arrList = new ArrayList<Object>();
		Object obj = null;
		for (String colName : colNames) {
			obj = po.get_Value(colName);
	...
	public static String getSqlUpdate (Integer AD_Table_ID)
	{
		if (loadSqlUpdate(AD_Table_ID)) {
			return s_SqlPoUpdate.get(AD_Table_ID);
		} else {
			return null;
		}
	}
	 ...
	 * @return
	 */
	public static Boolean loadSqlUpdate (Integer AD_Table_ID)
	{
		Boolean retValue = true;
		String sqlUpdate = s_SqlPoUpdate.get(AD_Table_ID);
		if (sqlUpdate != null && !"".equals(sqlUpdate.trim())) {
			return retValue;
		}
		MTable m_tbl = MTable.get(Env.getCtx(), AD_Table_ID);
		if (m_tbl == null) {
			return false;
		}
		ArrayList<String> arrLst = new ArrayList<String>();
		
		String sql = "select c.ColumnName from AD_Column c where"
				+ " c.AD_Table_ID = ? "
				+ " and c.ColumnSQL is null and c.isactive = 'Y'"
				+ " and c.ColumnName <> ? ";
		String columnKey = m_tbl.getTableName() + "_ID";
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		...
		if (count > 0) {
			StringBuilder sb = new StringBuilder();
			sb.append("update ");
			sb.append(m_tbl.getTableName());
			sb.append(" set ");
			sb.append(tm.toString());
			sb.append(" where " + columnKey + "=?");
			arrLst.add(columnKey);
			s_SqlPoUpdate.put(AD_Table_ID, sb.toString());
			s_SqlPoUpdate_Para.put(AD_Table_ID, arrLst);
		} else {
			retValue = false;
		}
	...
	public static List<String> getSqlUpdate_Para (Integer AD_Table_ID)
	{
		if (loadSqlUpdate(AD_Table_ID)) {
			return s_SqlPoUpdate_Para.get(AD_Table_ID);
		} else {
			return null;
		}
	}
	...
	@SuppressWarnings("unchecked")
	public static List<String> getSqlInsertOnly_Para(int AD_Table_ID) {
        if(loadSqlInsertOnly(AD_Table_ID)){
            return s_SqlPoInsert_ParaOnly.get(AD_Table_ID);
        }else{
            return null;
        }
	...
	private static boolean loadSqlInsertOnly(int AD_Table_ID) {
        Boolean retValue = true;
        String sqlInsert = s_SqlPoInsertOnly.get(AD_Table_ID);
        ...
        colNames.append(columnKey);
        tm.append(getSequenceName(m_tbl.getTableName()));
        ...
        if (count > 0) {
            ...
            sb.append("(");
            sb.append(colNames);
            sb.append(") values(" + tm.toString() + ")");
            s_SqlPoInsertOnly.put(AD_Table_ID, sb.toString());
            s_SqlPoInsert_ParaOnly.put(AD_Table_ID, arrLst);
        } else {
            retValue = false;
        }
        return retValue;
	...
	private static CCache<Integer, String> s_SqlPoInsert = new CCache<Integer, String>("SqlPoInsert", "SqlPoInsert", 5,  0,false);
	
	@SuppressWarnings("rawtypes")
	private static CCache<Integer, List> s_SqlPoInsert_Para = new CCache<Integer, List>("SqlPoInsert_Para", "SqlPoInsert_Para", 5, 0, false);
	
	
	public static Boolean loadSqlInsert(Integer AD_Table_ID, String trxName){
		Boolean retValue = true;
		String sqlInsert = s_SqlPoInsert.get(AD_Table_ID);
		if(sqlInsert!=null && !"".equals(sqlInsert.trim())){
			return retValue;
		}
		MTable m_tbl = MTable.get(Env.getCtx(), AD_Table_ID);
		...
		ArrayList<String> arrLst = new ArrayList<String>();
		StringBuilder colNames = new StringBuilder();
		String sql = "select c.ColumnName from AD_Column c where c.AD_Table_ID = ? and c.ColumnSQL is null and c.isactive = 'Y'";
		PreparedStatement pstmt = null;
        ResultSet rs = null;
        StringBuilder tm = new StringBuilder();
        int count = 0;
		...
		if (count > 0) {
			...
			sb.append(colNames);
			sb.append(") values(" + tm.toString() + ")");
			s_SqlPoInsert.put(AD_Table_ID, sb.toString());
			s_SqlPoInsert_Para.put(AD_Table_ID, arrLst);	
		}else{
			retValue = false;
		}
		return retValue;
	...
	@SuppressWarnings("unchecked")
	public static List<String> getSqlInsert_Para(Integer AD_Table_ID, String trxName){
		if(loadSqlInsert(AD_Table_ID, trxName)){
			return s_SqlPoInsert_Para.get(AD_Table_ID);
		}else{
			return null;
		}
	...
	public static List<Object> getBatchValueList(PO po, List<String> colNames, Integer AD_Table_ID, String trxName, int ID){
		...
		po.set_ValueNoCheck("UpdatedBy", Env.getAD_User_ID(Env.getCtx()));
		po.set_ValueNoCheck("Created", new Timestamp(new Date().getTime()));
		po.set_ValueNoCheck("Updated", new Timestamp(new Date().getTime()));
		po.setIsActive(true);
		
		Object obj = null;
		for(String colName : colNames){
			if(KeyColumnName.equals(colName)){
				obj = ID;
			}else{
				obj = po.get_Value(colName);
				if (obj instanceof Boolean)
	...
	public static String getSqlInsert(Integer AD_Table_ID, String trxName){
		if(loadSqlInsert(AD_Table_ID, trxName)){
			return s_SqlPoInsert.get(AD_Table_ID);
		}else{
			return null;
		}
	}
