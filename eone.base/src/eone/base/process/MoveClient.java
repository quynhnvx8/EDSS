...
public class MoveClient extends SvrProcess {
	...
	private void validate() {
		if (p_IsCopyClient) {
			...
			if (cntCN > 0)
				throw new EONEUserError("Client with name " + p_ClientName + " already exists in database");
			int cntCV = DB.getSQLValueEx(get_TrxName(), "SELECT COUNT(*) FROM AD_Client WHERE Value=?", p_ClientValue);
			if (cntCV > 0)
				throw new EONEUserError("Client with search key " + p_ClientValue + " already exists in database");		
		}
		...
		if (noClients <= 0) {
			throw new EONEUserError("No clients to move");
		}
		if (p_errorList.size() > 0) {
			return;
		}
		...
		if (! p_excludeTablesWhere.toString().contains("'AD_ATTACHMENT'")) {
			...
			if (cntES > 0) {
				throw new EONEUserError("There are attachments using external storage provider - that's not implemented yet");
			}
		}
		...
		if (! p_excludeTablesWhere.toString().contains("'AD_ARCHIVE'")) {
			...
			if (cntEA > 0) {
				throw new EONEUserError("There are archives using external storage provider - that's not implemented yet");
			}
		}
	...
	private void validateExternalTable(String tableName) {
		...
		if (! "AD_Client".equalsIgnoreCase(tableName)) {
			...
			if (cntCD > 0 && "AD_Attribute_Value".equalsIgnoreCase(tableName)) {
				throw new EONEUserError("Table " + tableName + " has data, migration not supported");
			}
		}
	...
	private void validateExternalColumn(String tableName, String columnName, int refID, int length) {
		...
		if (refID == DisplayType.SingleSelectionGrid || refID == DisplayType.MultipleSelectionGrid) {
			...
			if (cntMI > 0) {
				...
				throw new EONEUserError("There is data in unsupported Multi-ID column " + tableName + "." + columnName);
			}
		}
		if (!p_IsSkipSomeValidations && refID > MTable.MAX_OFFICIAL_ID) {
			...
			if (cntET > 0) {
				...
				throw new EONEUserError("There is data in unsupported non-official data type for column " + tableName + "." + columnName);
			}
		}
		...
		for (MTable table : tables) {
			...
			try {
				...
				while (rsGI.next()) {
					...
					if (targetID >= 0) {
						//DB.executeUpdateEx(insertConversionId, new Object[] {getAD_PInstance_ID(), tableName.toUpperCase(), sourceID, targetID}, get_TrxName());
					}
				}
			} catch (SQLException e) {
					...
					for (int i = 0; i < ncols; i++) {
						...
						if (! Util.isEmpty(convertTable)) {
							...
							if (p_IsCopyClient) {
								...
								*/
								if ("AD_Client".equalsIgnoreCase(tableName) && "Value".equalsIgnoreCase(columnName)) {
	...
	private int getFromUUID(String foreignTable, String uuidCol, String tableName, String columnName, String foreignUU, int foreignID) {
		...
		//DB.executeUpdateEx(insertConversionId, new Object[] {getAD_PInstance_ID(), foreignTable.toUpperCase(), foreignID, localID}, get_TrxName());
