...
public class ColumnSync extends SvrProcess
{
	...
	protected String doIt() throws Exception
	{
		...
		if (p_AD_Column_ID == 0)
			throw new EONEUserError("@No@ @AD_Column_ID@");
		MColumn column = new MColumn (getCtx(), p_AD_Column_ID, get_TrxName());
		if (column.get_ID() == 0)
			throw new EONEUserError("@NotFound@ @AD_Column_ID@ " + p_AD_Column_ID);
		
		MTable table = new MTable(getCtx(), column.getAD_Table_ID(), get_TrxName());
		if (table.get_ID() == 0)
			throw new EONEUserError("@NotFound@ @AD_Table_ID@ " + column.getAD_Table_ID());
		...
		try {
			...
			if (no == -1)
			{
				...
				msg.append(sql);
				throw new EONEUserError (msg.toString());
			}
			return sql;
		} finally {
			DB.close(rs);
