	...
	protected String doIt () throws Exception
	{
		...
		try
		{
			...
			while (rs.next ())
			{
				...
				if (DisplayType.isNumeric(AD_Reference_ID))
					length = 100;
				if (DisplayType.isLookup(AD_Reference_ID) || DisplayType.isID(AD_Reference_ID))
					length = 200;
				if (DisplayType.isText(AD_Reference_ID))
					length = 300;
				if (DisplayType.isDate(AD_Reference_ID))
					length = 100;
				field.setDisplayLength(length);
				...
				if (column.getColumnName().equals("AD_Client_ID")) {
					if (! accessLevel.equals(MTable.ACCESSLEVEL_Client)) {
						field.setIsDisplayedGrid(false);
						field.setIsDisplayed(false);
					}
				}
