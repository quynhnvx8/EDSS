...
import eone.base.model.MLookupCache;
...
public final class Env
{
	...
	public static final String AD_ORGACCESS_ID = "#AD_OrgAccess_ID";

	public static final String AD_CLIENT_ID = "#AD_Client_ID";
	
	public static final String MATERIAL_POLICY = "#MaterialPolicy";
	...
	public static void setContext (Properties ctx, int WindowNo, int TabNo, String context, String value)
	{
		...
		if (WindowNo == -1) {
			ctx.setProperty(TabNo+"|"+context, value);
		}
	}	//	setContext
	...
	//Lay chinh sach xuat vat tu hang hoa theo cau hinh. Set gia tri nay o RolePanel.java
	public static String getMaterialPolicy (Properties ctx)
	{
		return Env.getContext(ctx, MATERIAL_POLICY);
	}
	
	
	public static boolean checkRemainQty() {
		MClient client  = MClient.get(Env.getCtx(), Env.getAD_Client_ID(Env.getCtx()));
		if (client.isCheckMaterial())
			return true;
		return false;
	}
	
	public static int getBatchSize (Properties ctx)
	{
		return MSysConfig.getIntValue(MSysConfig.BATCH_SIZE, 100);
	}
	...
	public static String getListOrgAccesss (Properties ctx)
	{
...
//			url = Adempiere.class.getResource("images/" + fileName+".png");
    ...
    public static Timestamp getContextAsDateEx(Properties ctx, int WindowNo,
            int TabNo, String context)
    {
        if (ctx == null || context == null)
            throw new IllegalArgumentException ("Require Context");
        String s = getContext(ctx, WindowNo, TabNo, context, false);
        //  JDBC Format YYYY-MM-DD  example 2000-09-11 00:00:00.0
        if (s == null || s.equals(""))
        {
    ...
    public static Timestamp getContextAsDate(Properties ctx, int WindowNo,
            int TabNo, String context)
    {
        if (ctx == null || context == null)
            throw new IllegalArgumentException ("Require Context");
    ...
    public static BigDecimal getRateByCurrency(int C_Currency_ID, Timestamp DateAcct) {
    	String sql = "";
    	if (DB.isOracle() ) {
    		sql = "select ValueNumber from (select ValueNumber from c_currencyRate where DateStart <= ? And C_Currency_ID = ? order by datestart desc)b where ROWNUM <= 1";
    	} else {
    		sql = "select ValueNumber from (select ValueNumber, COUNT(*) OVER ( ORDER BY DateStart Desc) as ROWNUM from c_currencyRate where DateStart <= ? And C_Currency_ID = ? order by datestart desc)b where ROWNUM <= 1";
    	}
    	List<Object> param = new ArrayList<Object>();
    	param.add(DateAcct);
    	
    	param.add(C_Currency_ID);
    	BigDecimal rate = DB.getSQLValueBD(null, sql, param);
    	if (rate != null) {
    		return rate;
    	}
    	return Env.ZERO;
    ...
    //Context
    public static boolean DisProduct 			= false;
    public static boolean DisContract 			= false; //Hien thi hop dong
    public static boolean DisContractLine 		= false;//Hien thi contract line
    public static boolean DisProject 			= false;//Hien thi du an
    ...
    /**
	 * Ham tinh toan cong thuc toan hoc: VD: (1+2)*3 - 3 = 6
	 * Add 01/10/2020
	 * @param str
	 * @return
	 */
	...
	public static int getScaleFinal() {
		...
		return client.getRoundFinal();
	}
	
	public static int getScalePrice() {
		MClient client = MClient.get(getCtx());
		return client.getRoundUnitPrice();
	}
	
	public static int getScaleCalculating() {
		MClient client = MClient.get(getCtx());
		return client.getRoundCalculate();
	}
	/**
	 * Ham convert so thanh ky tu
	 */
	public static String numToChar(int num, int length) {
		String str = ""+ num;
